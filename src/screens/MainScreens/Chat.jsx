...
const Chat = () => {
  ...
  const createConversation = async (e) => {
    ...
    if (exists === true) {
      handleModal();
      alert("Coversation already exists with user");
    } else {
      const convo = {
        senderId: user._id,
        ...
        <div className="hidden w-full sm:block sm:w-[30%] border-r-[2px] border-gray">
          ...
          {loading ? (
            ...
            <div className="flex p-2 items-center ">
              <p className="font-bold text-[black] ">No Conversations yet</p>
            </div>
          )}
        </div>
        ...
        <div className="modal-box w-[400px] bg-white">
          ...
          {newChatArray.length !== 0 ? (
            ...
            <div className="flex h-[80px] flex-row items-center justify-between  rounded-[20px] bg-[white]  p-4">
              <p className="font-bold text-[black] text-lg">No Followers Yet</p>
            </div>
          )}
        </div>
      </dialog>
